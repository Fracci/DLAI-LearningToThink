"""
plot_probes.py — generate publication-quality figures from probe_results.xlsx.
...
All figures are saved as 300-dpi PNGs next to the workbook.
"""
import os
...
def load_data():
    """Read All_Probes into {pretty_label: {layer, trained, floor, gap, chance}}."""
...
def fig_gap_by_layer(data):
    """Headline: gap-over-floor vs layer for every probe, peak starred."""
...
def fig_trained_vs_floor(data):
    """Small multiples: trained accuracy vs random floor per probe."""
...
def fig_peak_gap_bars(data):
    """Bar chart of each probe's peak gap, annotated with the layer it peaks at."""
    labels = [l for l in STYLE if l in data]
...
def fig_carry_focus(data):
    """Carry-specific: trained/floor (left) and gap (right) for carry_in & gen_dist."""
