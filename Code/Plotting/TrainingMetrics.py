"""
TrainingMetrics.py — headline figures for the transfer (fine-tuning) results.
Reads full_results.xlsx, results_summary.xlsx, and per-seed
training-log CSVs (loss); no training/eval logic runs here.

Produces the training-evolution trajectories — OOD accuracy vs epoch per arm
against the shared random baseline with seed bands — plus the loss curves and
the 6-digit gap spectrum. Headline metric is exact-match (EM); per-digit (PD)
is the diagnostic. Noisy curves are drawn faint with a bold smoothed overlay
for readability.
"""
import os
import glob
...
SMOOTH_W = 5             # centered moving-average window for the bold overlay
...
# Per-seed training logs for the LOSS figure (loss isn't in the xlsx).
LOSS_LOGS = {
...
def smooth(ys, w=SMOOTH_W):
    ...
    (no padding artifacts). No-op if the series is too short or w<=1."""
...
def load_all_results():
    """Read the All_Results sheet of full_results.xlsx (raw per-seed/epoch EM &
    PD) into data[model][eval_set][metric][epoch] -> list of values across seeds."""
...
def mean_std_by_epoch(epoch_map):
    """Collapse {epoch: [values across seeds]} into sorted (epochs, means, stds)."""
...
def _plot_arm(ax, eps, means, stds, color, baseline=False, band=True):
    """Draw one arm's trajectory: a faint raw-mean line, a bold smoothed
    overlay, and (optionally) a shaded ±seed-std band — the shared readability
    convention used across all trajectory figures below."""
...
def PRETTY_LABEL(color):
    """Reverse-lookup a COLORS entry's pretty-printed model name from its hex
    color, so _plot_arm doesn't need the model string threaded through."""
...
def fig_trajectories(data, metric="EM"):
    """2x2 grid: accuracy vs epoch for id/5/6/7-digit eval sets, all four arms
    overlaid (faint-raw + bold-smoothed, ±seed-std band) — the full training-
    evolution view for one metric (EM or PD)."""
...
def fig_6dig_focus(data):
    """The decisive length in isolation: 6-digit EM (left) and PD (right) vs
    epoch, all arms — a zoomed-in companion to fig_trajectories."""
...
def fig_indist_mastery(data):
    """In-distribution EM vs epoch, all arms — the control figure showing all
    arms (including baseline) converge to near-100%, so the OOD gap can't be
    attributed to differences in how well training data itself was learned."""
...
def load_loss():
    """Glob and read the per-seed training-log CSVs named in LOSS_LOGS ->
    {model: {epoch: [loss over seeds]}}."""

    out = {}

    for model, (pat, col) in LOSS_LOGS.items():
        files = sorted(glob.glob(os.path.join(INDIR_LOSS, pat)))
        if not files:
            continue
...
def fig_loss(loss):
    """Training cross-entropy loss vs epoch, log-scale y, all arms with ±seed-std bands."""
...
def load_gaps():
    """Read the Gap_vs_Baseline sheet of results_summary.xlsx into
    {(model, ood, metric): (gap_mean, gap_std)}."""
...
def fig_gap_spectrum(gaps):
    """Headline bar chart: 6-digit EM (left) and PD (right) gap over baseline
    for the three pretrained arms — the compatibility spectrum result, with
    seed-std error bars and the mean labelled on each bar."""
...
def main():
    """Entry point. NOTE: only fig_loss is currently active — the other five
    calls are commented out below. Uncomment as needed to regenerate the full
    trajectory/6dig-focus/in-dist-mastery/gap-spectrum figure set."""
