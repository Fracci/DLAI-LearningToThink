"""
FreeRunEval.py — evaluates free-running (non-teacher-forced, autoregressive
greedy generation) accuracy across all four finetuned models (Rule30/Rollout/
Carry/Baseline) x all seeds x all OOD digit lengths, alongside a teacher-forced
eval on the EXACT SAME operand pairs, so the TF-minus-free "drop" is a pure
error-accumulation signal rather than an artifact of comparing different problems.
"""
import random
import csv
import os
...
def build_prompt_and_truth(tok, n1, n2, max_seq_len):
    """Encode just the 'n1+n2=' prompt (no scratchpad) plus the ground-truth
    sum string, for feeding into free-running generation."""
...
def generate(model, prompt_ids, tok, device, max_new=MAX_NEW_TOKENS):
    """Greedy-decode up to max_new tokens starting from the prompt, appending
    the model's own argmax prediction back into the context each step."""
...
def digit_pd(pred, truth):
    """Per-digit match count between predicted and true answer strings,
    aligned from the RIGHT (units digit) so length mismatches from a
    malformed/short generation still score partial credit correctly."""
...
def parse_answer(ids, tok):
    """Extract the digit string after 'A:' from a decoded generation; returns
    "" if the model never produced an 'A:' markersuch rows are NOT parseable and 
    deflate parseable_pct below."""
...
def eval_freerun(model, tok, d, problems, device):
    """Autoregressively generate an answer for every problem (no scratchpad
    ground truth fed in), scoring EM, per-digit PD, and parseable_pct (how
    often an 'A:' marker even appeared)."""
...
def load_model(path, vocab, device):
    """Load a finetuned checkpoint at whatever vocab size IT was trained with."""
...
def mean_std(xs):
    """Population mean/std, NaN-safe for empty input, 0-std for n=1."""
...
def main():
    """Sweep every model x seed x OOD length, running both free-run and
    matched teacher-forced eval on the same operand set, and write per-run
    detail plus a seed-averaged EM/PD summary."""
    ...
    def block(title, free_agg, tf_agg):
        """Print (and collect as rows) a model x digit-length grid of
        free-run/TF/drop means, one block per metric (EM, then PD)."""
